...
public:
    void adauga(const T& element) { elemente.push_back(element); }

    T& getElem(size_t index)
    {
        if (index < elemente.size())
            return elemente[index];
        else throw EroareAcces(static_cast<int>(index), static_cast<int>(elemente.size()));
    }


    [[nodiscard]] size_t getDimensiune() const { return elemente.size(); }

    void stergeElem(size_t index)
    {
        if (index >= elemente.size()) throw EroareAcces(static_cast<int>(index), static_cast<int>(elemente.size()));
